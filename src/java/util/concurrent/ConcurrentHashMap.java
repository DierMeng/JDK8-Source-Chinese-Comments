 ...
 *  1.链表长度 >= 8时，链表会转换为红黑树，<= 8 时又会恢复成链表；
 *  2.1.7 及以前，链表采用的是头插法，1.8 后改成了尾插法；
 *  3.Segment + ReentrantLock 改成了 CAS + synchronized。取消 Segment，直接利用 table 数组单元作为锁，实现了可对每行数据加锁，进一步提高了并发性能。
 *
